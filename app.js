...
// ============ STATE ============
  ...
  function renderAbout(){
    ...
    if(IMAGES && IMAGES.length){
      ...
      if(img && img.url){
        ...
        document.getElementById('aboutImg').alt = img.title || 'Trường Mầm non Diễn Lợi';
      }
    }
    // Cập nhật mô tả với thông tin thật
    var cfg = STATS.config || {};
    var name = cfg.name || 'Trường Mầm non Diễn Lợi';
    var addr = cfg.address || 'Xã Tân Châu, Tỉnh Nghệ An';
  ...
  function _doPrintMinhChung(){
    ...
    const schoolName = (cfg.name || 'Trường Mầm non Diễn Lợi').toUpperCase();
    ...
    // Tách địa chỉ: ["Xã Tân Châu", "Tỉnh Nghệ An"] → UBND lấy phần đầu (xã), location line cũng chỉ lấy xã (bỏ tỉnh)
